...
REM ###########################################################################
REM #  Copyright (c) 2014-2026 libbitcoin-explorer developers (see COPYING).
REM #
...
if "!libbitcoin_explorer_OWNER!" == "" (
    set "libbitcoin_explorer_OWNER=libbitcoin"
)
if "!libbitcoin_explorer_TAG!" == "" (
    set "libbitcoin_explorer_TAG=master"
)
...
:main
    ...
    set "REMAPPED_PARAMS=!REMAPPED_PARAMS! !libbitcoin-explorer_PARAMS!"
    ...
    call :source_github "!libbitcoin_explorer_OWNER!" "libbitcoin-explorer" "!libbitcoin_explorer_TAG!"
    if %ERRORLEVEL% neq 0 (
        exit /b %ERRORLEVEL%
    )
    call :build_msbuild "libbitcoin-explorer" "builds\msvc\%proj_version%" "libbitcoin-explorer" "!REMAPPED_PARAMS!"
    if %ERRORLEVEL% neq 0 (
        exit /b %ERRORLEVEL%
    )
    call :build_msbuild "libbitcoin-explorer" "builds\msvc\%proj_version%" "bx" "!REMAPPED_PARAMS!"
    if %ERRORLEVEL% neq 0 (
        exit /b %ERRORLEVEL%
    )
    if not "!BUILD_SKIP_TESTS!" == "yes" (
        call :build_msbuild "libbitcoin-explorer" "builds\msvc\%proj_version%" "libbitcoin-explorer-test" "!REMAPPED_PARAMS!"
        if %ERRORLEVEL% neq 0 (
            exit /b %ERRORLEVEL%
        )
    )
...
:display_constants
    ...
    call :msg "libbitcoin_explorer_OWNER       : !libbitcoin_explorer_OWNER!"
    call :msg "libbitcoin_explorer_TAG         : !libbitcoin_explorer_TAG!"
    exit /b %ERRORLEVEL%
