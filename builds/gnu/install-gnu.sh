...
#
###############################################################################
# Script managing the build and installation of libbitcoin-explorer and its dependencies.
#
# Script options:
...
if [[ -z ${libbitcoin_explorer_OWNER} ]]; then
    libbitcoin_explorer_OWNER="libbitcoin"
fi
if [[ -z ${libbitcoin_explorer_TAG} ]]; then
    libbitcoin_explorer_TAG="master"
fi
...
main()
{
    ...
    libbitcoin_explorer_FLAGS=()

    libbitcoin_explorer_OPTIONS=(
        "${with_boost}"
        "${with_pkgconfigdir}")
    ...
    source_github "${libbitcoin_explorer_OWNER}" "libbitcoin-explorer" "${libbitcoin_explorer_TAG}"
    local SAVE_CPPFLAGS="${CPPFLAGS}"
    export CPPFLAGS="${CPPFLAGS} ${libbitcoin_explorer_FLAGS[@]}"
    build_gnu "libbitcoin-explorer" "builds/gnu" "builds/gnu" "${PARALLEL}" "${libbitcoin_explorer_OPTIONS[@]}" "${CONFIGURE_OPTIONS_GNU[@]}"
    if ! [[ "${BUILD_SKIP_TESTS}" == "yes" ]]; then
        test_gnu "libbitcoin-explorer" "builds/gnu" "${PARALLEL}"
    fi
    install_gnu "libbitcoin-explorer" "builds/gnu"
    if [[ "${BUILD_POST_INSTALL_CLEAN}" == "yes" ]]; then
        clean_gnu "libbitcoin-explorer" "builds/gnu"
    fi
    export CPPFLAGS="${SAVE_CPPFLAGS}"
...
display_constants()
{
    ...
    msg "libbitcoin_explorer_OWNER       : ${libbitcoin_explorer_OWNER}"
    msg "libbitcoin_explorer_TAG         : ${libbitcoin_explorer_TAG}"
}

help()
{
    msg "Script managing the build and installation of libbitcoin-explorer and its dependencies."
    msg ""
    msg "Script options:"
